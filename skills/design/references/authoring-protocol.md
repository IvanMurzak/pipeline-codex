...
2. **If no generic pipeline fits, fall back to a regular Codex subagent.** Call `spawn_agent` without an agent type, with `fork_turns: "none"` and a bounded task message. Don't invent a new single-use pipeline just to satisfy a "use step-executor" request.
...
**Script conventions:** when you write a script as part of designing a new pipeline (whether called from inside an agent step — this principle — or as the whole `type: script` step of Principle 10), follow the conventions in `../../run/references/roles/pipeline-script-creator.md` (resolve it relative to this file) — pathlib for paths, stdlib only by default, argparse + `--help`, exit codes documented, idempotent, cross-platform, and a stdlib-`unittest` test file under `scripts/tests/` (a script is software; it ships with tests). Read that file once at the start of a design session if you anticipate any extractions; its rules are mandatory whenever you, the improver, or the script-creator worker author a script in this system.
