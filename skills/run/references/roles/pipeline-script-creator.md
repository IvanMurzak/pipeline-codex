...
where `<pipeline-root>` is the `Pipeline root:` the brief names — normally `<project-cwd>/.pipeline/<pipeline-name>/`. **On an `isolation: external` run the brief's pipeline root is the run WORKTREE's pipeline copy** (e.g. `<project>/.codex/worktrees/<run>/.pipeline/<pipeline-name>/`): write the script and the iteration rewrite THERE — never "correct" paths back to the main checkout; your work rides the run's own finalize commit/PR.

Never touch:

- Files outside the brief's `.pipeline/<pipeline-name>/` tree (consumer code, other docs, CI, tests, the consumer's `AGENTS.md`, etc.).
- The MAIN checkout's pipeline tree when the brief targets an external run's worktree copy (and vice versa — one run, one tree).
- Files inside `${CODEX_PLUGIN_ROOT}` (the plugin install directory is read-only at runtime).
...
4. **Move the step's declaration into the manifest.** In a v2 pipeline (`pipeline.yml`) the step's kind and inputs are manifest keys, not markdown — **and you must not write them yourself.** `pipeline.yml` is frozen against automated edits: it declares `timeout`, `needs`, `isolation`, i.e. what the run DOES, and changing that is a human's decision. So instead:
   - **Report the exact YAML** the operator should paste into the step's entry — `type: script`, `script: scripts/<name>.py` (pipeline-root-relative), `timeout:` (seconds; default 600 — a safe ceiling for the work), `retries:` (only for `transient`-class flakiness), `on_failure:` per the **mutating/read-only** rule, and the `params:` / `output:` blocks derived from the brief's Proposed Params (bindings like `${steps.<step-name>.output.<path>}`, `${run.id}`, `${env.<NAME>}`, `${pipeline.root}` for values earlier steps produce).
   - **Trim the body** to what an agent would still need: keep `Goal` and `Success Criteria` (they document the step for humans and for the fallback agent an `on_failure: agent` dispatches), and keep ONE human-readable `## Steps` line naming the script. Delete any `## Next` — the manifest owns the order.
   - In a **v1** pipeline there is no manifest, so write the same declaration into the step file's frontmatter and keep its single-path `## Next` (exactly one absolute path or `Pipeline complete.`; conditional prose there is a plan ERROR).
...
6. **Re-read what you wrote** and confirm the trimmed body still stands alone, and that the YAML you are reporting is complete — an operator should be able to paste it and run `pipeline plan` with zero errors.
...
6. **Do NOT touch the step's contract** — its `params:`, `output:` and every other manifest key stay exactly as they are (and `pipeline.yml` is frozen to you regardless). A contract change is a `convert-step` / designer job, not a repair.
