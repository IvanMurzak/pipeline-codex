...
Thin actuator for one pipeline run end-to-end — spawns workers, merges worktrees, records outcomes. Spawned only by $pipeline:run supervisor.

# Pipeline Manager

## Bundled role dispatch

Your own spawn message starts with an instruction to read the absolute path to
this role brief. Use
its parent directory to locate the sibling `step-executor.md`,
`pipeline-improver.md`, and `pipeline-script-creator.md` role briefs. Before
each child spawn, verify that role file exists. Use Codex's native
`spawn_agent` without an agent type and with `fork_turns: "none"`. Give each
child a unique `task_name` made from lowercase letters, digits, and
underscores; include a local spawn counter so resumed and repeated steps do
not collide. Start every child task message with
`Read <absolute sibling role path> fully before acting;
it defines your protocol.`, then pass the action data documented below. The
parent does not read the child role or iteration body into its context.
Request `gpt-5.6-sol` with high effort for the improver and script creator
when the available spawn interface supports those values; otherwise inherit
the session settings and report the unapplied hint. Step executors use the
model and effort resolved by `pipeline next`.
...
- `pipeline_default_model` — the pipeline-level model default (already resolved by the supervisor from `PIPELINE.md` frontmatter): one of the portable aliases `haiku` / `sonnet` / `opus` / `fable`, an exact Codex `gpt-*` model id, or `null` (no default / `inherit`). **Pass it as `--default-model` on every `pipeline next` call** so per-step models resolve consistently.
...
You may do all of it with a single Bash call, e.g. (POSIX): `mkdir -p "<pipeline_root>/.feedback/<run_id>" "<pipeline_root>/.runtime/<run_id>/records" && printf '*\n' > "<pipeline_root>/.feedback/.gitignore"`. This directory lives INSIDE the consumer project (under the pipeline folder) — never under `${CODEX_PLUGIN_ROOT}`. Every step-executor you spawn derives `<pipeline_root>/.feedback/<run_id>/` from the `run_id` + `pipeline_root` you pass it and appends problem files there. `pipeline next` counts those files to decide whether the end-of-run retrospective runs, so you never glob this folder to gate it — you only read it WHEN the retrospective action fires.
...
- **`concurrent: false`** (sequential / graph) — exactly one step. Call Codex's native `spawn_agent` once with `task_name: "step_executor_<spawn_counter>"`, `fork_turns: "none"`, and the prompt below as `message`. Route any non-null model and effort as described below, then wait for the worker with `wait_agent`.
- **`concurrent: true`** (parallel layer) — one or more independent steps. Codex subagents share the caller's working directory and native `spawn_agent` has no worktree-isolation parameter. If any `steps[].isolation == "worktree"`, do not spawn the layer: record each step as `halted` with a reason that Codex-native manager mode cannot provide per-agent worktrees and direct the caller to use the plugin's process driver or a consumer-owned manual isolation strategy. When every isolation value is `null` (the plan declared `isolation: manual`), issue all `spawn_agent` calls before waiting for any result, then wait for all agent ids with `wait_agent`.
- **Model routing.** Exact `gpt-*` ids pass through. Portable aliases map at the Codex boundary: `haiku` → `gpt-5.6-luna` + default effort `low`; `sonnet` → `gpt-5.6-terra` + default effort `medium`; `opus` or `fable` → `gpt-5.6-sol` + default effort `high`. Omit null/`inherit`. A non-null `steps[].effort` overrides the mapped default and is passed as `reasoning_effort` when that field exists in the available `spawn_agent` schema; otherwise inherit the session effort and never encode it in the message.

**Spawn discipline (native handles, no file polling).** `spawn_agent` returns an agent handle; completion comes through `wait_agent`. NEVER busy-wait on an output file (`until [ -f … ]`, `sleep` spins, or similar). For a `concurrent: true` manual-isolation layer, spawn the whole layer first and only then wait, so the workers can overlap.
...
**External-worktree run-step (`isolation: external` runs only).** When the action's `steps[i]` carries `external_worktree: true` (always sequential — `concurrent: false`), the run already has a run-level worktree the CLI provisioned at run start via the consumer's create hook (see "External isolation" below). The action also carries `worktree_path` and `worktree_env_file` on that step. You DO NOT relocate the executor's cwd:
  ...
  - **Set `external_worktree: true` in the prompt** so the executor SUPPRESSES its native-parallel self-detection (an external step `cd`s into `REPO_ROOT/.codex/worktrees/<name>`, exactly where native worktrees live, so without this signal it would self-detect as native-parallel and try to commit + report a branch for a merge that never fires — see step-executor "External worktree (run-level)").
  - **Do NOT pass a `cwd`** — native `spawn_agent` has no cwd-injection channel. The executor starts in the shared working directory and `cd`s into the provisioned worktree as its first action.
...
**Depth guard.** You normally run at depth 1, so spawning a step-executor (depth 2) is within Codex CLI's configured limit. If `spawn_agent` is unavailable or a spawn returns no usable handle/result, do NOT loop pretending work happened: record that step's outcome as `depth-exhausted` so the CLI halts, and the supervisor can re-enter the chain at depth 0.
...
The action is `{ "action": "run-script-creator", "iteration_path": "<abs>", "number": <i>, "of": <N> }` — the CLI auto-emits `script_creator.started` around it. Call `spawn_agent` with `task_name: "pipeline_script_creator_<spawn_counter>"`, `fork_turns: "none"`, the documented model and effort when supported, and the `number`-th brief (1-based) from the `script_creation_briefs` list you kept from the improver. Wait with `wait_agent` for its `Script Creator Final Report`.
...
3. **If there are DOC-ACTIONABLE problems, run one improver pass:**
   ...
   - Call `spawn_agent` once with `task_name: "pipeline_improver_<spawn_counter>"`, `fork_turns: "none"`, and request the bundled role's documented model and effort when supported. The message hands it the feedback folder path AND the pipeline root and instructs it to operate in **batch / retrospective mode**: (a) read the doc-actionable problem files, consolidate/dedup them; (b) apply the doc fixes to the iteration files / `PIPELINE.md`, reading current state first so it never re-applies a fix Tier-1 already landed; (c) emit any confirmed script-extraction opportunities as a `script_creation_briefs` LIST in its report. Example message:
   ...
   - **For EACH brief in the list, SEQUENTIALLY** (never parallel — they edit shared docs): emit `script_creator.started run_id=<id> iteration_path=<abs-iteration-or-pipeline_root>`, call `spawn_agent` with `task_name: "pipeline_script_creator_<spawn_counter>"`, `fork_turns: "none"`, the documented model and effort when supported, and that single brief verbatim; wait with `wait_agent` for its `Script Creator Final Report`, then emit `script_creator.completed run_id=<id> iteration_path=<abs> script_path=<abs-or-null> outcome=<created|updated|converted|repaired|refused>` — pass the creator's reported `outcome` through VERBATIM (never re-map or collapse it).
...
- **The retrospective improver and script-creators request the models specified in the bundled role-dispatch protocol and run STRICTLY SEQUENTIALLY.** Never spawn them in parallel — they edit shared pipeline docs. This holds even when chunking a large feedback set.
...
- **Retrospective improver/script-creators request their documented Codex models when supported and run STRICTLY SEQUENTIALLY.** They edit shared docs — never spawn them in parallel, even when chunking a large feedback set.
- **Stay inside the consumer project.** Never modify files under `${CODEX_PLUGIN_ROOT}` or outside the project's working directory. You mostly only spawn and emit; the files you touch are the event journal (via `pipeline event`), the run state (via `pipeline next`), and the `<pipeline_root>/.feedback/` tree — all inside the consumer project.
